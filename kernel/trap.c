...
usertrap(void)
{
  ...
  if(r_scause() == 8){
    ...
    // lazy page allocation: on a load/store page fault on an address
    // below p->sz, allocate the page on demand.
    uint64 sc = r_scause();
    uint64 va = r_stval();
    if(sc == 13 || sc == 15){
      ...
      if(pte != 0 && (*pte & PTE_V) && (*pte & PTE_COW)){
        ...
        // a page fault inside an mmap region: load the page from the file;
        // otherwise do lazy page allocation (only below p->sz).
        if(mmapfault(PGROUNDDOWN(va)) != 0){
          if(va < p->sz){
            char *mem = kalloc();
            if(mem == 0){
              setkilled(p);
